...
internal class TextLectureUploadStateHolder(
    ...
    fun handlePickedFiles(files: List<PickedUploadFile>) {
        ...
        feedbackMessage = null
        when (selectedTab) {
            UploadTab.File -> handlePickedPdf(files)
            UploadTab.Image -> handlePickedImages(files)
            UploadTab.Text -> files.releaseAll()
        }
    }
    ...
    fun removeFile(index: Int) {
        when (selectedTab) {
            UploadTab.File -> {
                selectedPdfFiles.getOrNull(index)?.release()
                selectedPdfFiles = selectedPdfFiles.filterIndexed { fileIndex, _ -> fileIndex != index }
            }

            UploadTab.Image -> {
                selectedImageFiles.getOrNull(index)?.release()
    ...
    private fun handlePickedPdf(files: List<PickedUploadFile>) {
        val file = files.firstOrNull()
        files.drop(1).releaseAll()
        when {
            file == null -> Unit
            file.sizeBytes > MaxPdfUploadBytes -> {
                file.release()
                showError("PDF는 최대 50MB까지 업로드할 수 있어요.")
            }

            else -> {
                selectedPdfFiles.releaseAll()
                selectedPdfFiles = listOf(file)
            }
        }
    }

    private fun handlePickedImages(files: List<PickedUploadFile>) {
        val remaining = (MaxImageUploadCount - selectedImageFiles.size).coerceAtLeast(0)
        val accepted = mutableListOf<PickedUploadFile>()
        var oversized = false
        var countExceeded = false

        files.forEach { file ->
            when {
                file.sizeBytes > MaxImageUploadBytes -> {
                    oversized = true
                    file.release()
                }

                accepted.size >= remaining -> {
                    countExceeded = true
                    file.release()
                }
        ...
        when {
            countExceeded -> showError("이미지는 최대 ${MaxImageUploadCount}장까지 업로드할 수 있어요.")
            oversized -> showError("이미지는 장당 최대 15MB까지 업로드할 수 있어요.")
        }
    }

    private fun releaseSelectedFiles() {
        selectedPdfFiles.releaseAll()
        selectedImageFiles.releaseAll()
    }
...
private fun List<PickedUploadFile>.releaseAll() {
    forEach(PickedUploadFile::release)
}
