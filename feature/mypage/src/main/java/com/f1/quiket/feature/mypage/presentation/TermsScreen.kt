...
fun TermsScreen(
    ...
    val sections = remember { ServiceTermsRawText.toTermsSections() }
    val title = remember { ServiceTermsRawText.lines().firstOrNull().orEmpty() }
    ...
    Box(
        ...
        Column(
            modifier = Modifier
                .fillMaxSize()
                .padding(top = 52.dp, bottom = 124.dp)
...
internal data class TermsSectionData(
    val title: String,
    val body: String,
)

internal fun String.toTermsSections(): List<TermsSectionData> {
    val sections = mutableListOf<TermsSectionData>()
...
private val ServiceTermsRawText = """
[만 14세 이상] 서비스 이용 약관
제1조 (목적)
이 약관은 퀴켓(Quiket)(이하 '회사'라고 합니다)가 제공하는 제반 서비스의 이용과 관련하여 회사와 회원과의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.
제2조 (정의)
...
'도토리'란 회원이 출석, 학습 활동, 이벤트 참여 등 회사가 정한 적립 정책에 따라 부여받는 서비스 내 가상 크레딧을 말하며, 그 적립·사용·소멸에 관한 구체적인 사항은 별도의 조항 및 운영정책에서 정합니다.
...
제4조 (약관의 효력과 변경)
이 약관은 퀴켓(Quiket)(이)가 제공하는 모든 인터넷서비스에 게시하여 공시합니다. '회사'는 '전자상거래 등에서의 소비자보호에 관한 법률(이하 '전자상거래법'이라 함)', '약관의 규제에 관한 법률(이하 '약관규제법'이라 함)', '전자문서 및 전자거래 기본법(이하 '전자문서법'이라 함)', '전자금융거래법', '정보통신망 이용촉진 및 정보보호 등에 관한 법률(이하 '정보통신망법'이라 함)', '소비자기본법' 등 관계 법령(이하 '관계법령'이라 함)에 위배되지 않는 범위 내에서 이 약관을 변경할 수 있으며, 회사는 약관이 변경되는 경우에 변경된 약관의 내용과 시행일을 정하여, 그 시행일로부터 최소 7일(이용자에게 불리하거나 중대한 사항의 변경은 30일) 이전부터 시행일 후 상당한 기간 동안 공지하고, 기존 이용자에게는 변경된 약관, 적용일자 및 변경사유(변경될 내용 중 중요사항에 대한 설명을 포함)를 별도의 전자적 수단(전자우편, 문자메시지, 서비스 내 전자쪽지발송, 알림 메시지를 띄우는 등의 방법)으로 개별 통지합니다. 변경된 약관은 공지하거나 통지한 시행일로부터 효력이 발생합니다.
...
AI 기반 학습 퀴즈 생성 및 풀이 서비스
이용자가 업로드한 학습 자료(강의 노트, 이미지, 텍스트 등)의 텍스트 추출 및 자동 분류 서비스
회원 정보 및 학습 이용내역에 기반한 맞춤형 학습 기록·통계 제공 서비스
도토리(가상 크레딧) 적립·사용 및 기타 이용자의 편의를 위하여 제공하는 서비스
제14조 (서비스의 제한 등)
...
제19조 (도토리 등)
회원은 출석, 학습 활동, 회사 또는 제휴회사에서 주최하는 이벤트 참여 등 회사의 적립 정책에 따라 도토리(가상 크레딧)를 부여받게 됩니다.
도토리는 서비스 내에서만 사용 가능하며, 현금으로 환급되거나 양도될 수 없습니다.
도토리는 회사가 정한 사용기간이 만료되거나 이용계약이 종료(회원 탈퇴 등)되는 경우 소멸합니다.
도토리의 적립·사용·소멸에 관한 구체적인 기준은 회사의 운영정책에서 정하며, 회사는 운영정책에 따라 그 기준을 변경할 수 있습니다.
제20조 (권리의 귀속)
회사가 제공하는 서비스에 대한 저작권 등 지식재산권은 회사에 귀속됩니다.
회사는 서비스와 관련하여 이용자에게 회사가 정한 조건에 따라 회사가 제공하는 서비스를 이용할 수 있는 권한만을 부여하며, 이용자는 이를 양도, 판매, 담보제공 하는 등 처분행위를 할 수 없습니다.
...
제1항에 불구하고 회사는 다음 각 호의 목적으로 별도의 허락 없이 무상으로 저작권법 및 공정한 거래관행에 합치되는 범위 내에서 회원이 등록한 콘텐츠를 사용할 수 있습니다.
서비스 내에서 이용자가 작성한 콘텐츠의 복제, 수정, 전시, 전송, 배포 등 저작권을 침해하지 않는 범위 내의 2차적저작물 또는 편집 저작물 작성을 위한 사용
서비스 품질 개선 및 신규 서비스 개발(AI 모델의 학습 및 고도화를 포함합니다)을 위한 사용. 이 경우 회사는 개인을 식별할 수 없도록 비식별·익명 처리한 형태로 콘텐츠를 활용합니다.
전항에도 불구하고 해당 콘텐츠를 등록한 이용자가 콘텐츠의 삭제 또는 사용 중지를 요청하는 경우, 회사는 관련 법에 따라 보존해야 하는 사항을 제외하고 관련 콘텐츠를 모두 삭제 또는 사용 중지합니다.
제23조 (관할법원 및 준거법)
서비스와 관련하여 분쟁이 발생한 경우 관할법원은 회사 소재지 관할법원으로 정하며, 준거법은 대한민국의 법령을 적용합니다.
부칙
제1조 (시행일). 본 약관은 2026.06.17.부터 시행됩니다.
""".trimIndent()
