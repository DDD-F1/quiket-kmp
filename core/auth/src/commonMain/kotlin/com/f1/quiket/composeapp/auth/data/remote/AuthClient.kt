...
import com.f1.quiket.composeapp.network.decodeApiEnvelope
import com.f1.quiket.composeapp.network.ensureTrailingSlash
import com.f1.quiket.composeapp.network.requireSuccessful
...
internal class AuthClient(
    ...
    suspend fun kakaoLogin(
        ...
        val envelope = response.decodeAuthEnvelope()
    ...
    private suspend fun decodeEmptyResponse(
        ...
        response.decodeAuthEnvelope().requireAuthSuccess(
            statusCode = response.status.value,
            failureMessage = failureMessage,
        )
    }

    private suspend fun HttpResponse.decodeAuthEnvelope(): ApiEnvelope =
        try {
            decodeApiEnvelope(json = json)
        } catch (error: ApiException) {
            throw AuthException(error.message.orEmpty())
        }

    private fun ApiEnvelope.requireAuthSuccess(
        statusCode: Int,
        failureMessage: String,
    ) {
        try {
            requireSuccessful(statusCode = statusCode, failureMessage = failureMessage)
        } catch (error: ApiException) {
            val errorData = toAuthErrorData(json)
            throw AuthException(
                message = error.message.orEmpty(),
